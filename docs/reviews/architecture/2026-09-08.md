...
5. **Configuration via ~15 constructor kwargs** with no central config object
   (except valkey) — a blurred config/DI boundary.
...
|---|---|---|
| Three levels: BasicAsyncWorker → AsyncTaskProcessor → ValkeyWorker | ✅ | `basic_async_worker.py`, `async_tasks_processor.py`, `valkey/valkey_async_worker.py` |
...
| Shared-queue topology (AR-023) | ✅ | `valkey_async_worker.py:110-118`: stream/group service-scoped, consumer/status/registry worker-scoped |
...
- **Severity:** MEDIUM
- **Evidence:** `valkey/valkey_async_worker.py:428-492` — `purge_tasks()`,
...
- **Evidence:** `valkey/valkey_config.py:282-325` `generate_glide_config(...,
  ...
  (`valkey_async_worker.py:120`). The branch is documented as "reserved for
...
- **Severity:** LOW
- **Evidence:** `valkey/valkey_async_worker.py:1-20` and `valkey/valkey_config.py:1-20`
...
- **Severity:** LOW
- **Evidence:** `valkey/valkey_async_worker.py:297` `heartbeat()` — guard
...
- **Severity:** LOW
- **Evidence:** `valkey/valkey_async_worker.py` `_task_entry_ids` is populated in
...
- **Severity:** LOW
- **Evidence:** `valkey/valkey_async_worker.py:513` `_recover_pending_tasks` —
...
- **Severity:** LOW
- **Evidence:** `valkey/valkey_async_worker.py` `heartbeat()` (broad except →
...
Clean. The only nuance is the duplicated glide import-guard (AR-048). The
PubSub branch in `generate_glide_config` (AR-044) is now a documented,
tested capability for custom workers.
...
7. **AR-048** — move the glide import-guard into a single module.

### Priority 3 (extensibility and observability)
8. **AR-046** — introduce a typed configuration object.
...
- `task_manager` woken by an event, not polling (AR-042) — **done** via batched
  ...
  not the bottleneck.
- Configuration — a single typed object (AR-046).
...
1. **Phase 1 (safe cleanups):** remove dead code (AR-043/044), extract the
   glide import-guard (AR-048), remove write side effects (AR-047). Each is a
   separate commit with green tests. — AR-043 **done** (2026-09-08): `purge_tasks`
   moved to the standalone `purge_task_stream` utility.
...
4. **Phase 4 (structure):** removal of forwarding wrappers (AR-045) and
   introduction of a configuration object (AR-046) — only in the next major
   version (v5), since it changes the public constructor.
...
| AR-046 | Config via kwargs, no central object | MEDIUM | Scalability | L | P3 | open |
| AR-047 | Write side effects in read functions | MEDIUM | Least surprise | S | P2 | open |
| AR-048 | Duplicated glide import-guard | LOW | Maintainability | S | P2 | open |
...
| AR-052 | Handler remains on stop timeout | LOW | State | S | P3 | open |
| AR-053 | Handlers keyed only by class name | LOW | Extensibility | M | P3 | open |
| AR-054 | Broad except Exception in best-effort | LOW | Diagnosis | S | P3 | open |
| AR-055 | Low queue/concurrency defaults | LOW | Performance | S | P3 | open |
