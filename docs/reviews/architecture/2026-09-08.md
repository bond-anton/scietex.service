...
Dependency direction is clean: the core (`BasicWorker` →
`TaskProcessor`) never depends on `valkey`/`glide`; `ValkeyWorker` extends
...
|---|---|---|
| Three levels: BasicWorker → TaskProcessor → ValkeyWorker | ✅ | `basic_worker.py`, `task_processor.py`, `valkey/worker.py` |
| Core does not depend on valkey/glide | ✅ | `valkey/__init__.py` imported only from the package `__init__.py` under an `ImportError` guard; `task_processor.py` does not import valkey |
...
1. **Clean dependency direction.** The core knows nothing about valkey.
   `ValkeyWorker` is an overlay on `TaskProcessor`. This is the correct
...
- **Resolution:** Fixed via an overridable hook
  ...
  `task_processor.py`. The base default requeues a drained task when
...
- **Confidence:** HIGH
- **Resolution:** Removed all 9 forwarding wrappers from `basic_worker.py`
  (`_iter_manager_definitions`, `_register_logger_handler`,
  `_logger_start_handlers`, `_logger_shut_down_handlers`, `_run_manager`,
  `_start_manager`, `_stop_manager`, `_start_managers`, `_stop_managers`).
  Internal callers now delegate directly to the extracted components:
  `__init__` registers the `ConsoleHandler` via
  `LoggingLifecycle.register_logger_handler`, `_startup`/`_shutdown` call
  `LoggingLifecycle.start_handlers`/`shut_down_handlers` and
  `ManagerRuntime.start_managers`/`stop_managers` directly, and
  `ValkeyWorker._ensure_logging_handler` registers the `AsyncValkeyHandler` via
  `LoggingLifecycle.register_logger_handler`. Tests repointed to
  `worker._manager_runtime`; the AR-049 test seam now wraps
  `_manager_runtime.start_managers` instead of overriding the removed
  `_start_managers`. Landed in the unreleased v4.0.0.
...
- **Resolution:** Introduced a typed configuration object. `WorkerConfig` and
  ...
  a single config object — `BasicWorker(config: WorkerConfig | None)`,
  `TaskProcessor(config: TaskProcessorConfig | None)`,
...
- **Recommendation:** Set `start_time` before the managers start (or use a
  ...
  ValkeyWorker.
- **Confidence:** HIGH
- **Resolution:** `_startup` in `basic_worker.py` now sets `start_time` before
  the managers start, so the heartbeat manager's immediate first beat is written
  instead of being skipped by the `self.start_time` guard in `heartbeat()`. A
  ValkeyWorker test (`test_first_heartbeat_writes_status_key_promptly`) covers
  the `start_time` guard path.
...
- **Confidence:** MEDIUM
- **Resolution:** `cleanup()` in `valkey/worker.py` now clears `_task_entry_ids`
  after the parent `TaskProcessor.cleanup()` drains the queue and cancels
  running tasks, so entries for cancellation-ignoring handlers no longer leak
  across stop/start cycles. Clearing is done in `cleanup()` (the shutdown path
  after task cancellation), not in `disconnect()` (which is also called from
  other contexts).
...
- **Confidence:** MEDIUM
- **Status:** ✅ RESOLVED (2026-09-09)
- **Resolution:** `_recover_pending_tasks` now returns `(recovery_complete, enqueued)` and `fetch_tasks` only sets `_recovered = True` when recovery fully drains the pending list, so a queue-full or error interruption is retried on the next poll instead of leaving pending entries unclaimed until restart.
...
- **Resolution:** `add_task_handler` in `task_processor.py` now takes an
  optional keyword-only `name` parameter (defaults to the handler class name),
  enabling multiple instances of one class under distinct lifecycle keys.
  Because each instance is constructed with its resolved name, a subclass can
  derive `supported_tasks` from `self.name` and split one class's task types
  across instances; dispatch selects by `supported_tasks` membership (first
  match wins), so per-instance task sets must not overlap. Duplicate resolved
  keys still raise `ValueError` (backward-compatible).
...
logging are factored into subpackages. The weak spot is `BasicWorker`
...
5. **AR-045** — remove/deprecate the forwarding wrappers in `BasicWorker`. —
   **DONE** (2026-09-09): all 9 forwarding wrappers removed; internal callers
   delegate to `ManagerRuntime`/`LoggingLifecycle` directly.
...
**Target changes:**
- `BasicWorker` — a thin facade without proxy wrappers (AR-045) — **done**
  (2026-09-09): all 9 forwarding wrappers removed; internal callers delegate to
  `ManagerRuntime`/`LoggingLifecycle` directly.
...
4. **Phase 4 (structure):** removal of forwarding wrappers (AR-045) — **done**
   (2026-09-09): all 9 wrappers removed and internal callers delegate to
   `ManagerRuntime`/`LoggingLifecycle` directly, landed in the still-unreleased
   v4.0.0; the configuration object (AR-046) — **done** (2026-09-09): it landed
   in the still-unreleased v4.0.0, so the constructor change is acceptable now
   rather than deferred to v5.
...
| AR-045 | Blown-up BasicWorker + proxy layer | MEDIUM | Maintainability | M | P2 | ✅ resolved |
...
| AR-049 | First ValkeyWorker heartbeat skipped | LOW | Observability | S | P3 | ✅ resolved |
| AR-050 | _task_entry_ids not cleared | LOW | Memory leak | S | P3 | ✅ resolved |
| AR-051 | Early return from pending recovery | LOW | Reliability (edge) | S | P3 | ✅ resolved |
| AR-052 | Handler remains on stop timeout | LOW | State | S | P3 | ✅ resolved |
| AR-053 | Handlers keyed only by class name | LOW | Extensibility | M | P3 | ✅ resolved |
