...
- **Resolution:** `cleanup()` in `valkey/worker.py` now clears `_task_entry_ids`
  after the parent `AsyncTaskProcessor.cleanup()` drains the queue and cancels
...
**Target changes:**
- `BasicAsyncWorker` — a thin facade without proxy wrappers (AR-045) — **done**
...
| AR-045 | Blown-up BasicAsyncWorker + proxy layer | MEDIUM | Maintainability | M | P2 | ✅ resolved |
