...
- **Date:** 2026-09-16
- **Version:** v4.1.0 (`src/scietex/service/version.py:6`) — the unreleased v4.1.0 line; the v4.2/v4.3 commit labels were a message error, since corrected (AR-010, resolved).
...
> **All findings resolved (2026-09-16).** AR-001..AR-014 were addressed in the
> v4.2.0 migration across five phases: Phase 1 AR-003 (`635acbf`), Phase 2
> AR-002 (`635acbf`), Phase 3 AR-001 (`f406bad`), Phase 4 AR-004/005/014
> (`510daad`), Phase 5 AR-006..009/011..013 (`e346cc6`). AR-010 was already
...
**What to fix first:** AR-001 (make the transport contract an explicit
interface/component instead of a hook soup), then AR-002/AR-003 (extract the
...
and documented in the *base* class docstrings (AR-001). A second, minor leak is
...
heartbeat, and instance registry (AR-002). The lease subsystem and the
...
`_validate_range` helper from the core `config.py` (AR-008).
...
the **template-method transport contract** (AR-001): instead of a single
...
The remaining concurrency concerns are narrow (AR-006 lease/requeue race,
AR-012 stale-client window).
...
public injection seam (AR-003). The `valkey` unit tests are excellent *despite*
...
transport extensibility (AR-001): adding a second transport (the stated goal
...
### AR-001 — The transport extension contract is an implicit, ordering-sensitive hook soup
...
### AR-002 — `ValkeyWorker` is a large, low-cohesion component; lease and tracking sub-systems are inlined
...
- **Why it matters:** High cognitive load and merge/review friction on the
  ...
  this one object for every concern (AR-003).
- **Recommendation:** Extract a `TaskLeaseManager` (acquire/refresh/delete/held,
  ...
  reduces the risk of, the AR-001 transport extraction.
- **Confidence:** HIGH (factually multi-responsibility); MEDIUM (severity — the
  class works and is well-tested).

### AR-003 — No client-injection seam; `connect()` is a concrete factory, forcing white-box tests
...
- **Why it matters:** It is a dependency-inversion gap: the worker depends on a
  ...
  is coupled to internals that any AR-001/AR-002 refactor would move. It also
...
### AR-004 — Weak operational observability: no connection-health supervision
...
### AR-005 — Dual-mode config leaks a union type and forces two parallel translations
...
- **Why it matters:** The raw-`GlideClientConfiguration` fallback is the source
  of the remaining `handler.client` reach (AR-014) and the shared-client
...
### AR-006 — Per-entry lease: non-configurable derived TTL and a residual requeue/id-reuse race
...
### AR-007 — `_task_lease_held` is dead code
...
- **Why it matters:** Dead surface invites confusion about the lease design and
  is one more method to read in an already large class (AR-002).
...
### AR-008 — Cross-package import of a private helper (`_validate_range`)
...
### AR-009 — `None`-means-default config resolution uses two inconsistent mechanisms
...
### AR-010 — Version string (4.1.0) is stale relative to v4.2/v4.3 feature commits
...
### AR-011 — Handler capability injection is informal (magic `handler_kwargs`)
...
### AR-012 — `_client_lock` serializes `_client` mutation but not its use
...
### AR-013 — Manager discovery via MRO `__dict__` reflection is stringly-named and rename-fragile
...
### AR-014 — `ValkeyWorker` still mutates `handler.client` in the raw-config fallback
...
- **Recommendation:** Eliminate the raw-`GlideClientConfiguration` fallback or
  route it through the same `valkey_config=` seam (see AR-005).
- **Confidence:** HIGH.
...
The dominant leak is **transport semantics into the base class** (AR-001): the
...
logging-handler ownership split re-appears on the raw-config fallback (AR-014),
and config translation is duplicated (AR-005).

### Lifecycle / resource ownership
Worker and manager lifecycles are strong. Client ownership is single-owner and
lock-serialized for mutation, but *use* is unsynchronized (AR-012), and the
client has no observable health state (AR-004). The lease has a derived,
non-configurable TTL (AR-006).
...
concurrency risks (AR-006 requeue/id reuse, AR-012 stale client) are narrow and
consistent with the documented at-least-once + idempotency contract.

### Abstractions crossing package boundaries
Three: (1) the template-method transport seam (AR-001) — the biggest; (2) the
private `_validate_range` import (AR-008); (3) the `handler.client` reach on the
raw-config fallback (AR-014). All three are fixable by the same architectural
move (make the transport a first-class component).

### Observability
Transport health is the weak dimension: no connection state, no aggregate
error, only swallowed WARNINGs (AR-004). Manager health was fixed (AR-063);
...
1. **AR-001 — make the transport a first-class interface/component.** Extract a
...
2. **AR-003 — add a client/transport injection seam.** This is the
   lowest-risk enabler: it unblocks test de-whitening and is a prerequisite for
   AR-001/AR-002 without changing behavior.
3. **AR-002 — extract `TaskLeaseManager` and `TaskStatusStore`** as small
   collaborators, shrinking `ValkeyWorker` and isolating the lease/tracking
   invariants.

### Priority 2 — Important
4. **AR-004 — add a transport/connection health signal** (connected/degraded
   state + aggregate error, CRITICAL watchdog log when down).
5. **AR-005 — normalize config to one internal form** and derive the
...
6. **AR-006 — make `task_lease_ttl` configurable** (derived default) and
   document/close the requeue lease-clobber window.
7. **AR-007/AR-009 — hygiene:** remove `_task_lease_held`, converge the
   two `None`-resolution patterns. (AR-010 resolved: commit messages corrected,
   no version bump needed.)
8. **AR-008/AR-011/AR-013/AR-014** — promote `_validate_range`, consider a
   `TaskCapabilities` object when capabilities grow, and eliminate the
   raw-config `handler.client` reach as part of AR-005.
...
**Target changes:**
- A `Transport` interface owns `fetch`/`ack`/`requeue`/`release` (AR-001);
  `ValkeyTransport` implements it and owns the client, reconnect, intake,
  recovery, and acknowledgement (AR-001, AR-004, AR-012). The client is
  injectable via a factory seam (AR-003).
- `TaskLeaseManager` and `TaskStatusStore` become collaborators of
  `ValkeyTransport` (AR-002, AR-006).
- One normalized internal config form with the logging-handler config derived
  from it (AR-005, AR-014).
...
1. **Phase 1 — testability (no behavior change):** add a client/transport
   factory seam to `ValkeyWorker` construction (AR-003). De-white-box tests as
   the seam lands.
2. **Phase 2 — extraction (no behavior change):** extract `TaskLeaseManager`
   and `TaskStatusStore` from `ValkeyWorker` (AR-002). Keep the existing public
...
3. **Phase 3 — transport interface:** define the `Transport` interface and
   ...
   (AR-001). `ValkeyWorker` becomes thin composition of `TaskProcessor` +
...
4. **Phase 4 — observability + config normalization:** add the transport health
   signal (AR-004) and collapse the dual config mode (AR-005, AR-014). Depends
...
5. **Phase 5 — hardening/hygiene:** make `task_lease_ttl` configurable and
   document the requeue lease window (AR-006); remove `_task_lease_held`
   (AR-007); converge config resolution (AR-009);
   promote `_validate_range` (AR-008); add `TaskCapabilities` when the
   capability count grows (AR-011).
...
Phase 5 `e346cc6`. AR-011 was resolved by documenting the trade-off rather than
...
1. **Scale-out reality:** Is the multi-replica shared-queue model actually
   ...
   the practical severity of AR-006(b) and AR-012.
2. **Second transport timeline:** Is an MQTT/Kafka transport actually planned
   ...
   foreseeable future? This determines whether AR-001 is a near-term priority
   or a theoretical one.
3. **Raw-`GlideClientConfiguration` usage:** Is the raw-config fallback (the
   source of AR-005/AR-014) exercised by any real consumer, or can it be
...
| -- | ----- | -------- | ------ | ------ | -------- |
| AR-001 | Transport contract is an ordering-sensitive hook soup | HIGH | Extensibility/correctness | L | 1 | ✅ resolved |
| AR-002 | `ValkeyWorker` low cohesion; lease/tracking inlined | MEDIUM | Maintainability | M | 1 | ✅ resolved |
| AR-003 | No client-injection seam; white-box tests | MEDIUM | Testability/DI | S | 1 | ✅ resolved |
| AR-004 | No connection-health supervision | MEDIUM | Observability | M | 2 | ✅ resolved |
| AR-005 | Dual-mode config leaks union + two translators | MEDIUM | Maintainability | M | 2 | ✅ resolved |
| AR-006 | Lease TTL derived + requeue lease-clobber race | MEDIUM | Operability/edge correctness | S | 3 | ✅ resolved |
| AR-007 | `_task_lease_held` dead code | LOW | Clarity | S | 3 | ✅ resolved |
| AR-008 | Cross-package private `_validate_range` import | LOW | Boundary hygiene | S | 3 | ✅ resolved |
| AR-009 | Two inconsistent `None`-resolution patterns | LOW | Consistency | S | 3 | ✅ resolved |
| AR-010 | Version string stale (4.1.0 vs v4.2/v4.3) | LOW | Release provenance | S | 3 | ✅ resolved |
| AR-011 | Informal handler capability injection | LOW | Extensibility (future) | S | 3 | ✅ resolved |
| AR-012 | `_client_lock` protects mutation not use | LOW | Concurrency (residual) | S | 3 | ✅ resolved |
| AR-013 | MRO-reflection manager discovery rename-fragile | LOW | Maintainability | M | 3 | ✅ resolved |
| AR-014 | `handler.client` reach in raw-config fallback | LOW | Cross-package leak | S | 3 | ✅ resolved |
...
the transport seam (AR-001..AR-003), not a redesign: the correctness machinery
...
(AR-001), the `TaskLeaseManager`/`TaskStatusStore` extraction (AR-002), and the
`client_factory=` seam (AR-003); observability and config normalization followed
(AR-004/005/014), then the hardening/hygiene items (AR-006..009, 011..013).
