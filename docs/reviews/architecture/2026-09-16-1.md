...
| -- | ----- | -------- | ------ | ------ | -------- |
| AR-001 | Transport contract is an ordering-sensitive hook soup | HIGH | Extensibility/correctness | L | 1 |
| AR-002 | `ValkeyWorker` low cohesion; lease/tracking inlined | MEDIUM | Maintainability | M | 1 |
| AR-003 | No client-injection seam; white-box tests | MEDIUM | Testability/DI | S | 1 |
| AR-004 | No connection-health supervision | MEDIUM | Observability | M | 2 |
| AR-005 | Dual-mode config leaks union + two translators | MEDIUM | Maintainability | M | 2 |
| AR-006 | Lease TTL derived + requeue lease-clobber race | MEDIUM | Operability/edge correctness | S | 3 |
| AR-007 | `_task_lease_held` dead code | LOW | Clarity | S | 3 |
| AR-008 | Cross-package private `_validate_range` import | LOW | Boundary hygiene | S | 3 |
| AR-009 | Two inconsistent `None`-resolution patterns | LOW | Consistency | S | 3 |
| AR-010 | Version string stale (4.1.0 vs v4.2/v4.3) | LOW | Release provenance | S | 3 | ✅ resolved |
| AR-011 | Informal handler capability injection | LOW | Extensibility (future) | S | 3 |
| AR-012 | `_client_lock` protects mutation not use | LOW | Concurrency (residual) | S | 3 |
| AR-013 | MRO-reflection manager discovery rename-fragile | LOW | Maintainability | M | 3 |
| AR-014 | `handler.client` reach in raw-config fallback | LOW | Cross-package leak | S | 3 |
