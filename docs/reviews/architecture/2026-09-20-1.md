...
- **Status:** ✅ RESOLVED (2026-09-20) — documented as a scope boundary (D0): the single-worker-per-service constraint on control-plane commands is now stated in `AGENTS.md`, `docs/remote_config.md`, and `docs/task_processor.md`. Cross-worker control routing (a durable unified control stream per transport with per-worker and broadcast addressing, "D4") is planned for v5.0.0. Commit `pending`.
- **Severity:** MEDIUM
- **Evidence:**
...
1. **Is a service's task source intended to be single-worker or fleet-shared?** This determines whether AR-123 is a documentation fix or a routing design.
   - **Answered (D0 + D4):** the single-worker-per-service scope boundary is now documented (D0), and fleet-shared control routing is planned for v5.0.0 (D4).
...
|---|---|---|---|---|---|
| AR-123 | Control-plane commands not routed across a multi-worker fleet | MEDIUM | Cancel silently no-ops on the wrong worker; config reaches one worker | Low (document) / High (route) | 3 | ✅ resolved (documented) |

---

**Verdict:** The v4.5.0 control-plane work is correct for a single worker process. The multi-worker routing/fan-out gap (AR-123) is a scope boundary, not a regression: it is pre-existing, silent, and operational. The cheapest correct step is to document the single-worker contract; a fleet-routing design is a separate, larger effort to scope only if fleet-shared control is intended.

Succeeded: 0 findings resolved in this review (1 new finding raised: AR-123). Failed: 0. Skipped: 0.
