...
- **Method:** read all `docs/architecture/*` as an initial map (not authoritative), read the prior review (2026-09-16-1, AR-001..AR-014), verified every claim against the full source package (5433 lines) end-to-end, and confirmed evidence with targeted searches.
- **Type:** deep architectural review (read-only). The only repository change is this document.

> **Identifier note.** This review restarted the AR numbering at AR-001, so its
> `AR-015` collides with the `AR-015` of the 2026-09-04 series
> (`docs/reviews/architecture/2026-09-04.md:587`, "Worker construction is
> coupled to a running loop", LOW, resolved). The two are unrelated. Within
> this document, `AR-015` always means **"Manager discovery is reflection-based
> and rename-fragile"** (HIGH). Where the older issue is meant, it is written
> as `2026-09-04 AR-015`.

---

## Executive Summary

The architecture is **sound**. The v4.2.0 migration (AR-001..AR-014) closed the
...
1. **Manager discovery is reflection-based and rename-fragile** (AR-015). The
   ...
   **Resolved (2026-09-17, commit `a62985f`)** — see the `Status` line on
   AR-015 below.
2. **`BasicWorker` still owns too many distinct concerns** (AR-016): signal
...
3. **The `TaskProcessor` task lifecycle is spread across three managers and a
   `ContextVar`** (AR-017): `TaskManager` runs handlers, `TaskQueueManager`
...
**What to fix first:** AR-015 (make manager registration explicit), then
AR-016 (extract the worker state machine / signal handling), then AR-017
...
leaked in v4.1.0 is now an explicit Protocol (AR-001 resolved).
...
- `valkey/transport.py` imports four sibling modules (`config`, `health`,
  ...
  connection schema, health, and two keyed stores (AR-018).
...
registry hooks, and the logging-lifecycle delegation (AR-016). The manager
runtime owns manager *execution* but not manager *discovery*, which is
reflection over the worker class (AR-015).
...
`..config` was fixed in v4.2.0 (AR-008 resolved; now `_validation.validate_range`).
...
whose identity is the attribute name, discovered by MRO reflection (AR-015).
The `**handler_kwargs` capability-injection channel (`cancel=`, `report=`) is
an informal abstraction documented as an accepted trade-off (AR-011 resolved);
it remains a latent problem if a third capability appears (AR-020).
...
handling, logging, and registry calls (AR-016). The `lifecycle.md` map itself
...
`self.client` around awaits (AR-012 resolved) and added a single reconnect
...
**task lifecycle state** noted above (AR-017) and the **manager registry**,
which is derived rather than declared (AR-015).
...
requires the test to run inside the right task context (AR-017).
...
is its method name (AR-015). Extensibility at the handler boundary is good
...
- **Delivery semantics are correct and tested.** At-least-once, requeue-before-
  ack, lease-at-enqueue-accept, and the AR-006b lease-clobber fix are all
...
### AR-015 — Manager discovery is reflection-based and rename-fragile
...
- **Why it matters:** Renaming a decorated method silently renames the manager
  ...
  silently. The v4.2.0 review accepted this as a trade-off (AR-013) with an
...
### AR-016 — `BasicWorker` still owns the state machine, signals, and registry
...
- **Recommendation:** Extract a `WorkerLifecycle` (or `WorkerStateMachine`)
  ...
  manager registry (AR-015) rather than defined as methods.
...
### AR-017 — Task lifecycle state is distributed across four components
...
### AR-018 — The "transport" concept is split across core and the Valkey package
...
### AR-019 — Config search path is frozen at import time
...
### AR-020 — `**handler_kwargs` capability injection is informal
...
### AR-021 — The no-signal process-exit path is undefined
...
### AR-022 — `ValkeyWorker` still owns connection/config/logging-handler concerns
...
- **Why it matters:** `ValkeyWorker` is still the largest class in the package
  and still mixes concerns. The extraction is less urgent than AR-016 because
...
### AR-023 — `purge_task_stream` swallows all exceptions
...
### AR-024 — `TaskStatusStore.update_progress` synthesizes a record on miss
...
### AR-025 — `_validation.validate_range` has an `unbounded_ok` special case
...
### AR-026 — `TaskProcessor` auto-tunes `max_concurrent_tasks` from `os.cpu_count()`
...
### AR-027 — `TaskEnvelope` version handling is a single hard-coded branch
...
### AR-028 — `Manager.__get__` descriptor returns a bound method, hiding the decorator
...
- **Problem:** The descriptor makes the decorated attribute behave as a plain
  ...
  same reflection path as AR-015.
- **Why it matters:** It reinforces the reflection-based discovery: there is no
  ...
  contributing factor to AR-015 rather than an independent defect.
- **Recommendation:** Fold into the AR-015 fix: if managers are registered
...
`scietex.logging`. The one cross-layer observation is AR-018: `health.py` is
...
1. **Manager discovery leaks into the worker class layout** (AR-015): the
   runtime reflects over the worker's MRO to find managers.
2. **The state machine leaks into `BasicWorker`** (AR-016): lifecycle
   transitions are interleaved with signals, logging, and registry calls.
3. **Task lifecycle state leaks across four components** (AR-017): no single
...
remaining gaps are the undefined no-signal exit path (AR-021) and the
distributed task lifecycle state (AR-017).
...
transport, and two stores (AR-017).
...
`report_progress` to the call site's task context (AR-017).
...
crosses awkwardly is the transport *implementation* split (AR-018): the
...
- **AR-015 — Explicit manager registration.** Replace MRO reflection with a
  registry. This is the highest-leverage change: it removes the framework's
  most fragile implicit contract and unblocks AR-016 (the built-in managers
  should be registered, not defined as methods).
- **AR-016 — Extract the worker state machine and signal handling.** Apply the
...
- **AR-017 — Consolidate the task lifecycle state.** Introduce a single owner
  for a task's record and transitions; replace the `ContextVar` with an
  explicit handle.
- **AR-019 — Compute the `./config` candidate at call time.** A small, safe fix
  to a latent startup bug.
- **AR-021 — Specify the no-signal exit path.** Close the documented `UNKNOWN`
  and cover it with a test.

### Priority 3 — Opportunistic

- **AR-018 — Plan the transport layering.** Hoist generic health to core when a
  second transport is planned; document the intended layering now.
- **AR-020 — Introduce `TaskCapabilities` when a third capability appears.**
  Keep the current channel until then; document the two capabilities.
- **AR-022 — Extract `ValkeyConnection`/logging-handler factory.** Defer until
  `ValkeyWorker` changes again.
- **AR-023 — Return a result from `purge_task_stream`.**
- **AR-024 — Log or skip on synthesized progress records.**
- **AR-025 — Reconsider the `unbounded_ok` flag.**
- **AR-026 — Document the `auto_tune` heuristic.**
- **AR-027 — Distinguish unsupported version from malformed envelope.**
- **AR-028 — Fold into the AR-015 fix.**
...
The changes are independent enough to land incrementally, but AR-015 should go
first because AR-016 depends on it (the built-in managers move into the
registry).

1. **AR-015 — Manager registry.** Add the registry, make the decorator append
   ...
   its manager name.
2. **AR-016 — Worker lifecycle + signal handler.** Extract the state machine
   and signals. Verify: lifecycle tests pass; add a test for the no-signal path
   (AR-021).
3. **AR-019 — Config cwd fix.** Independent, small. Verify: a test that changes
   cwd after import and resolves `./config` correctly.
4. **AR-017 — Task lifecycle consolidation.** The largest change; do it after
   AR-015/AR-016 so the manager set is stable. Verify: all delivery-semantics
   tests pass unchanged.
5. **AR-018/AR-020/AR-022..AR-028 — Opportunistic.** Land as the relevant
   components change.

Dependencies between changes: AR-016 → AR-015; AR-017 → AR-015 (stable manager
set); AR-018 → a second-transport decision; the rest are independent.
...
1. **Is a second transport (MQTT/Kafka) actually planned?** This determines
   whether AR-018 is worth acting on now or should stay deferred.
2. **How often does the task lifecycle change?** AR-017's consolidation is
...
3. **Do any embedders drive the worker programmatically without signals?** This
   determines the severity of AR-021.
4. **Is `auto_tune` used in production?** This determines whether AR-026 is a
   documentation fix or a behavior change.
5. **Is the `./config` candidate relied upon?** This determines whether AR-019
...
|----|-------|----------|--------|--------|----------|
| AR-015 | Manager discovery is reflection-based and rename-fragile | HIGH | High — framework's most load-bearing implicit contract | Medium | 1 | ✅ resolved |
| AR-016 | `BasicWorker` owns state machine, signals, registry | HIGH | High — base class coupling propagates to all workers | Medium | 1 |
| AR-017 | Task lifecycle state distributed across four components | MEDIUM | Medium — reasoning and change cost | High | 2 |
| AR-018 | "Transport" concept split across core and Valkey | MEDIUM | Medium — blocks clean second transport | Medium | 2 |
| AR-019 | Config search path frozen at import time | MEDIUM | Medium — silent startup misresolution | Low | 2 |
| AR-020 | `**handler_kwargs` capability injection is informal | MEDIUM | Medium — untyped DI channel | Medium | 2 |
| AR-021 | No-signal process-exit path undefined | MEDIUM | Medium — embedder contract gap | Low | 2 |
| AR-022 | `ValkeyWorker` owns connection/config/logging concerns | LOW | Low — leaf class, same pattern | Medium | 3 |
| AR-023 | `purge_task_stream` swallows all exceptions | LOW | Low — operator tool | Low | 3 |
| AR-024 | `update_progress` synthesizes a record on miss | LOW | Low — silent data quality | Low | 3 |
| AR-025 | `validate_range` has an `unbounded_ok` special case | LOW | Low — small semantic leak | Low | 3 |
| AR-026 | `auto_tune` uses `os.cpu_count()` for an I/O workload | LOW | Low — opt-in heuristic | Low | 3 |
| AR-027 | Envelope version handling is a single hard-coded branch | LOW | Low — diagnostics only | Low | 3 |
| AR-028 | `Manager.__get__` hides the decorator | LOW | Low — contributes to AR-015 | Low | 3 |

**Resolution (2026-09-17):** AR-015 was addressed in commit `a62985f`
...
The remaining findings (AR-016..AR-028) are open.

Succeeded: 1 finding (AR-015). Failed: 0. Skipped: 0.
