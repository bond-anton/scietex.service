...
- **Status:** ✅ RESOLVED (2026-09-17) — the intended layering is now documented: `TransportHealth` is deliberately transport-agnostic and is slated to be hoisted to core when a second transport is added; commit `2c88212`.
- **Severity:** MEDIUM
...
- **Resolution (2026-09-17, commit `2c88212`):** the finding's own second recommendation was taken — the intended layering is now documented rather than accidental. `docs/architecture/components.md` §13 states that `TransportHealth` is deliberately transport-agnostic (it takes only injected callables and knows nothing about Valkey) and lives under `valkey/` only because the Valkey transport is currently the only transport; when a second transport is added it should be hoisted to core alongside the `TaskTransport`/`TaskSink` Protocols. `docs/architecture/overview.md` carries the same note. No code change was made: the contract is already correctly in core, the wire format (`task_handler/wire.py`) is already transport-agnostic, and hoisting `TransportHealth` now would break the public `scietex.service.valkey.health` import path for no current benefit.
...
- **Status:** ✅ RESOLVED (2026-09-17) — the exit contract is now specified in `docs/architecture/lifecycle.md` (replacing the former `UNKNOWN` marker) and pinned by two tests; no behavior change was required — the code was already correct by construction; commit `b3ada6f`.
- **Severity:** MEDIUM
...
- **Resolution (2026-09-17, commit `b3ada6f`):** `docs/architecture/lifecycle.md` now documents that the `exit` event is set iff an exit was requested (via `exit()` or a signal), that a bare `stop()` runs the full graceful shutdown but deliberately leaves `exit` unset, that `exit()` does not block, and that the library does not own loop/process exit (no `atexit`/loop-close hook) — with a per-exit-path table. `docs/basic_worker.md` step 7 was corrected to state the `exit` event is conditional, and the mislabeled "Wait for full startup" example was fixed. The `exit()` docstring in `basic_worker.py` no longer claims it waits for the worker to stop. Two tests were added to `tests/test_basic_worker.py`: `test_stop_alone_does_not_set_exit_event` and `test_exit_sets_exit_event_from_running`.
...
- **Status:** ✅ RESOLVED (2026-09-17) — the logging-handler config translation was moved out of `valkey/worker.py` into `valkey/config.py` as the public `logging_handler_config(valkey_config)` function, sitting beside its sibling `generate_glide_config`; the connection and config concerns were deliberately left in place; commit `5e4fa10`.
- **Severity:** LOW
...
- **Resolution (2026-09-17, commit `5e4fa10`):** `_logging_handler_config` was a pure module-level function with no worker state, so it was hoisted to `valkey/config.py` as `logging_handler_config` — the module that already owns the other `ValkeyConfig` translation (`generate_glide_config`). `ValkeyWorker` now imports it. The connection lifecycle and the deferred config read were deliberately NOT extracted: `_client` is the class's integration point, wired into four collaborators (`TransportHealth`, `TaskLeaseManager`, `TaskStatusStore`, `ValkeyTransport` via `client_provider` lambdas) plus four worker methods and ~26 direct `worker._client` test seams, and the `_ensure_client_config()` → `_ensure_logging_handler()` ordering inside `_connect_locked` is guaranteed only by their co-location under `_client_lock`. The review's premise that `ValkeyWorker` is "still the largest class" is also inaccurate — it is 599 lines, third behind `TaskProcessor` (910) and `BasicWorker` (682). Behavior is unchanged; the full suite passes at 294.

### AR-094 — `purge_task_stream` swallows all exceptions

- **Status:** ✅ RESOLVED (2026-09-17) — `purge_task_stream` now returns a frozen `PurgeResult` (`entries_purged` count plus an `errors` tuple) instead of `None`, so an operator can distinguish a full purge from a partial one; the log-and-never-raise behavior is preserved; commit `5c3123b`.
- **Severity:** LOW
...
- **Resolution (2026-09-17, commit `5c3123b`):** `PurgeResult` is a `@dataclass(frozen=True)` in `valkey/purge.py` with `entries_purged: int = 0` and `errors: tuple[str, ...] = ()`; the two internal helpers (`_purge_group_entries`, `_purge_stream_entries`) now return `int` counts; the single `except Exception` still logs ERROR and returns a partial result, so the utility never raises and `asyncio.CancelledError` still propagates. `PurgeResult` is exported from `scietex.service.valkey`. New tests: `tests/valkey/test_purge.py` (6 tests). Note the finding's evidence was slightly mislocated — the internal helpers never swallowed; the single catch-all was in `purge_task_stream` itself.

### AR-095 — `TaskStatusStore.update_progress` synthesizes a record on miss

- **Status:** ✅ RESOLVED (2026-09-17) — `update_progress` no longer fabricates a default `running` record when the tracking key is missing; the progress update is dropped and logged at DEBUG, so a missing record stays missing rather than becoming a plausible-but-wrong entry; commit `1bf4d30`.
- **Severity:** LOW
...
- **Confidence:** HIGH (mechanism verified; low impact).
- **Resolution (2026-09-17, commit `1bf4d30`):** the synthesize-on-miss branch in `src/scietex/service/valkey/tracking.py` was replaced with a DEBUG log and an early return; the read-modify-write path for a present record, the silent `DecodeError` return, the `client is None` return, and the transport-error WARNING + `report_failure` are all unchanged. New test `test_write_task_progress_missing_record_is_noop` in `tests/valkey/test_tracking.py` asserts the read still happens but nothing is written. Docs `docs/valkey_worker.md` and `docs/architecture/components.md` updated to describe the drop-on-miss behavior.

### AR-096 — `_validation.validate_range` has an `unbounded_ok` special case

- **Status:** ✅ RESOLVED (2026-09-17) — the `<= 0` "unbounded" convention was moved out of the generic `validate_range` helper into `TaskProcessorConfig.__post_init__`, and the `unbounded_ok` flag was removed from `validate_range` entirely; commit `1a94485`.
- **Severity:** LOW
...
- **Confidence:** HIGH (mechanism verified; low impact).
- **Resolution (2026-09-17, commit `1a94485`):** `validate_range` is now a pure range check (`None` short-circuit, lower-bound, upper-bound) with no domain-specific flag; `TaskProcessorConfig.__post_init__` guards the non-positive `task_timeout` sentinel before calling it. Behavior is unchanged and all existing tests pass without modification — the sentinel acceptance (`tests/test_config.py::test_task_timeout_unbounded_sentinel_accepted`), the `0.05` middle-band rejection, and `tests/task_processor/test_watchdog.py::test_watchdog_ignores_non_positive_configured_task_timeout` all still pin the semantics.

### AR-097 — `TaskProcessor` auto-tunes `max_concurrent_tasks` from `os.cpu_count()`

- **Status:** ✅ RESOLVED (2026-09-17) — the `auto_tune` heuristic's suitability caveat is now documented — the CPU count is a poor proxy for an I/O-bound asyncio workload and does not reflect container CPU limits, so I/O-bound services should set `max_concurrent_tasks` explicitly; commit `6700a0f`.
- **Severity:** LOW
...
- **Confidence:** HIGH (mechanism verified; low impact).
- **Resolution (2026-09-17, commit `6700a0f`):** the caveat was added to the `auto_tune` docstring in `TaskProcessorConfig` (`src/scietex/service/config.py`), the `auto_tune` table row in `docs/task_processor.md`, and the `auto_tune` paragraph in `docs/architecture/components.md`. No behavior change: the heuristic remains `max(1, os.cpu_count())` and the four existing tests (`tests/task_processor/test_config.py` ×3, `tests/valkey/test_lifecycle.py` ×1) still pin the exact formula. The finding's alternative recommendation (a configurable multiplier) was deliberately not implemented — it is a behavior change gated on Open Question 4.

### AR-098 — `TaskEnvelope` version handling is a single hard-coded branch

- **Status:** ✅ RESOLVED (2026-09-17) — a `decode_task_envelope_version` helper was added to `task_handler/wire.py`, and the two Valkey transport rejection sites now log the envelope version (or `malformed`) so an unsupported version is distinguishable from a corrupt payload; commit `de05766`.
- **Severity:** LOW
...
- **Confidence:** HIGH (mechanism verified; low impact).
- **Resolution (2026-09-17, commit `de05766`):** `decode_task_envelope_version(payload) -> int | None` peeks the envelope version without changing the public `decode_task_envelope -> TaskData | None` contract; `ValkeyTransport.fetch` and `recover_pending_tasks` now log the envelope version (or the literal `malformed`) in their rejection messages. Two tests were added in `tests/test_task_handler_wire.py` covering the unknown-version and malformed cases plus the supported-version case. The public contract, the two callers, and the four existing wire tests are unchanged.

### AR-099 — `Manager.__get__` descriptor returns a bound method, hiding the decorator

- **Status:** ✅ RESOLVED (2026-09-17) — folded into the AR-086 registry fix (commit `a62985f`): discovery no longer reflects over the MRO `__dict__`; `Manager.__get__` is retained purely for bound-method ergonomics and is not the discovery mechanism.
- **Severity:** LOW
...
- **Recommendation:** Fold into the AR-086 fix: if managers are registered
  ...
  discovery mechanism.
- **Confidence:** HIGH (mechanism verified; low impact).
- **Resolution (2026-09-17, commit `a62985f`):** the descriptor is unchanged and still returns a bound method on instance access, but the harm it described no longer applies: `ManagerRuntime.iter_manager_definitions()` reads `MANAGER_REGISTRY_ATTR` directly (`manager/runtime.py:90,123`) rather than reflecting over arbitrary class attributes, so the descriptor is not a discovery mechanism. This is exactly the outcome AR-099 recommended — keep the descriptor for ergonomics, register managers explicitly. No code change was required.
...
| AR-089 | "Transport" concept split across core and Valkey | MEDIUM | Medium — blocks clean second transport | Medium | 2 | ✅ resolved |
...
| AR-092 | No-signal process-exit path undefined | MEDIUM | Medium — embedder contract gap | Low | 2 | ✅ resolved |
| AR-093 | `ValkeyWorker` owns connection/config/logging concerns | LOW | Low — leaf class, same pattern | Medium | 3 | ✅ resolved |
| AR-094 | `purge_task_stream` swallows all exceptions | LOW | Low — operator tool | Low | 3 | ✅ resolved |
| AR-095 | `update_progress` synthesizes a record on miss | LOW | Low — silent data quality | Low | 3 | ✅ resolved |
| AR-096 | `validate_range` has an `unbounded_ok` special case | LOW | Low — small semantic leak | Low | 3 | ✅ resolved |
| AR-097 | `auto_tune` uses `os.cpu_count()` for an I/O workload | LOW | Low — opt-in heuristic | Low | 3 | ✅ resolved |
| AR-098 | Envelope version handling is a single hard-coded branch | LOW | Low — diagnostics only | Low | 3 | ✅ resolved |
| AR-099 | `Manager.__get__` hides the decorator | LOW | Low — contributes to AR-086 | Low | 3 | ✅ resolved |
...
state channel. AR-092 was addressed in commit `b3ada6f`: the no-signal exit
contract is now specified in `docs/architecture/lifecycle.md` (replacing the
`UNKNOWN` marker) and pinned by two tests, with no behavior change required.
AR-094 was addressed in commit `5c3123b`: `purge_task_stream` now returns a
frozen `PurgeResult` (`entries_purged` count plus an `errors` tuple) instead of
`None`, while the log-and-never-raise behavior is preserved. AR-095 was addressed
in commit `1bf4d30`: `TaskStatusStore.update_progress` no longer synthesizes a
default `running` record when the tracking key is missing — the progress update
is dropped and logged at DEBUG, so a missing record stays missing rather than
becoming a plausible-but-wrong entry. AR-096 was addressed in commit `1a94485`: the `<= 0` "unbounded" convention moved out of the generic `validate_range` helper into `TaskProcessorConfig.__post_init__`, and the `unbounded_ok` flag was removed. AR-097 was addressed in commit `6700a0f`: the `auto_tune` heuristic's suitability caveat is now documented in the config docstring and the task-processor and components docs — the CPU count is a poor proxy for an I/O-bound asyncio workload and does not reflect container CPU limits. AR-098 was addressed in commit `de05766`: a `decode_task_envelope_version` helper now peeks the envelope version, and the Valkey transport's two rejection sites log it (or `malformed`) so an unsupported version is distinguishable from a corrupt payload. AR-099 was addressed in commit `a62985f`: it was folded into the AR-086 registry fix — discovery reads `MANAGER_REGISTRY_ATTR` rather than reflecting over the MRO `__dict__`, so `Manager.__get__` is retained purely for bound-method ergonomics and is no longer a discovery mechanism. AR-089 was addressed in commit `2c88212`: the intended transport layering is now documented — `TransportHealth` is deliberately transport-agnostic and is slated to be hoisted to core when a second transport is added. AR-093 was addressed in commit `5e4fa10`: the logging-handler config translation moved from `valkey/worker.py` to `valkey/config.py` as `logging_handler_config`, beside its sibling `generate_glide_config`; the connection and deferred-config concerns were deliberately left in place because `_client` is the class's integration point. All findings are now resolved (none remain open).

Succeeded: 14 findings (AR-086, AR-087, AR-088, AR-089, AR-090, AR-091, AR-092, AR-093, AR-094, AR-095, AR-096, AR-097, AR-098, AR-099). Failed: 0. Skipped: 0.
