...
**Dependencies.** Direction is correct and acyclic. The `TaskTransport` Protocol is an *incomplete* seam: it declares a dead method (`release`) and omits the two lifecycle operations both workers actually invoke (`refresh_leases`, `recover_pending_tasks`), forcing concrete down-casts (AR-113).
...
- **Status:** ✅ RESOLVED (2026-09-18) — extracted `TaskExecutor` (`src/scietex/service/task_executor.py`) owning dequeue → track → dispatch → retry → ack, the timeout watchdog, cancellation, and shutdown drain; `TaskProcessor` composes it and keeps thin delegators (`task_manager`, `watchdog`, `_cancel_task`) plus the public API. No public API change; `TaskProcessor` dropped ~280 lines. New isolated `tests/task_executor/` suite added.
- **Severity:** HIGH
...
- **Status:** ✅ RESOLVED (2026-09-18) — extracted `TransportWorker` (`src/scietex/service/transport_worker.py`), a shared core base owning the client-lock/reconnect pattern (`connect`/`disconnect`/`_reconnect`), `TransportHealth` construction + `transport_health`, the startup config-apply pipeline (`_apply_local_config`/`_reload_remote_config`/`_read_remote_outcome`/`_log_config_outcome`), and the `watchdog` glue. `ValkeyWorker` and `MqttWorker` now extend it and keep only broker-specific `_connect_locked`/`_disconnect_locked`, `_read_remote_outcome`, `heartbeat`, `initialize`, `cleanup`, registry methods, and (MQTT) intake/message-loop methods. Exported from `__init__.py` (additive). New isolated `tests/transport_worker/` suite. Commit `2b9f66d`.
- **Severity:** HIGH
...
- **Status:** ✅ RESOLVED (2026-09-19) — a new restart-required `TaskProcessorConfig.max_timeout_requeues` (default 1, range 0..100) bounds the timeout-driven requeue loop via a distinct `TaskExecutor._timeout_requeues` budget; the watchdog requeues at most the ceiling then acks terminal. Commit `d6d6cb1`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-19) — extracted a `ConfigManager` collaborator (`src/scietex/service/config_manager.py`, internal, not exported) owning the `ConfigReloader`, the local `config.yml` path/reads/writes, the attached `ConfigSource`, and the three `config:*` handler callbacks; `TaskProcessor` keeps `_current_reloadable_settings`, `_restart_required_fields`, `_apply_reloadable_config` (the AR-100 atomic `_config`/`_effective` write stays on the owner), and the public `config_revision`/`config_hash`/`config_source`/`register_config_settings` delegators, dropping `_config_source`, `_config_reloader`, `_write_local_config`, `_config_apply`, `_config_store`, `_config_show`. Behavior change: the three `config:*` handlers are now registered only when `remote_config_enabled=True`; on a disabled worker they are absent, so a `config:*` task yields the permanent "No handler found for task type 'config:...'" result instead of a `REMOTE_CONFIG_DISABLED` outcome (`ConfigManager.show_config` retains its `REMOTE_CONFIG_DISABLED` guard for direct calls). Commit `e5c845c`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-19) — `WorkerLifecycle.state` is no longer a bare settable enum: state moves only through `transition(new_state)` (validated against an allowed-edge table; an illegal edge raises `InvalidStateTransition`) or the unguarded `force_stopped()` terminal escape. `_wait_until_stopped()` now awaits an `asyncio.Event` (`_stopped`, set iff the state is STOPPED) instead of polling every 0.1 s. `ManagerRuntime.stop_managers(reverse=True)` and `BasicWorker._stop_managers_best_effort()` (now called from both the `_startup` and `_shutdown` `CancelledError` handlers) unwind managers in reverse start order before forcing STOPPED, so a cancelled orchestrator no longer strands running managers under STOPPED. Commit `73afd28`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-19) — `Manager` now keeps only its decorator + descriptor roles; the registry-entry role moved to a descriptor-free `ManagerDefinition` value (`__slots__ = ("name", "method", "cleanup", "owner", "attribute_name")`), which is the only type stored in `MANAGER_REGISTRY_ATTR`. Both registration paths (`Manager.__set_name__` and `register_manager`) funnel through a single private `_record_definition(owner, definition, *, replace)` primitive, and `ManagerRuntime.iter_manager_definitions()` yields `(name, ManagerDefinition)` with `run_manager`/`start_manager` taking `ManagerDefinition`. `BasicWorker`'s built-in Heartbeat/Watchdog managers are now `@Manager`-decorated methods (`_heartbeat_manager`/`_watchdog_manager`) on the class rather than module-level free functions, and `register_manager` has no in-tree production caller and is a documented compatibility shim. Commit `be35564`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-19) — control-plane commands are split onto a priority lane: a new `CONTROL_TASK_TYPES` frozenset + `is_control_task` predicate (exported from `task_handler/__init__.py`) classify `cancel_task`/`config:*`; `TaskExecutor` gained a keyword-only `control_queue`/`control_concurrency` lane (`DEFAULT_CONTROL_CONCURRENCY = 4`) with `run_once` admitting control first and `_settle` balancing the correct lane; `TaskProcessor` routes `enqueue_task` through `is_control_task` into `__control_queue`, adds `control_queue_empty()`/`control_queue_full()`, and the `task_queue_manager` intake gate now polls when the data queue is full but the control lane has room; `max_concurrent_tasks` bounds the **data plane** only. `InMemoryTransport.fetch`, `MqttTransport.fetch`/`recover_pending_tasks`, and `ValkeyTransport.fetch` (bounded `_deferred` buffer + `_flush_deferred`) deliver control commands past a full data lane while preserving data FIFO/backpressure. Commit `741cc6e`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-20) — `TaskTracker` is now a `@dataclass(frozen=True, slots=True)` (`task_handler/runtime.py`), so the runtime handle can no longer be mistaken for a serializable value type; constructor, fields, equality, and the `task_handler` re-export are unchanged, and a regression test pins the dataclass contract. Commit `b7fe602`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-20) — the `ConfigSource` protocol docstring now defines `load()` as best-effort current desired state **without waiting for delivery**, with freshness provenance explicitly transport-inherent (Valkey live `GET` vs MQTT last recorded snapshot); both source `load` docstrings and `docs/remote_config.md` state the contract, and the MQTT `wait_for_snapshot` is documented as an intentional transport-specific startup primitive outside the protocol. The protocol surface stays exactly `{load, store}` (pinned by a test), so external structural implementers are unaffected. Commit `4453093`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-19) — `ConfigReloader.reset()` clears the run-scoped apply bookkeeping (revision/hash/source/section state) at each run boundary; `apply_envelope(trusted=True)` skips signature verification for the trusted local artifact so the re-encoded `revision=1` `config.yml` is no longer rejected as `STALE_CONFIG`; `ConfigManager.reset()` delegates to the reloader, `TaskProcessor.initialize()` calls `self._config_manager.reset()` at the run boundary, and `MqttConfigSource.reset()` clears its retained-topic snapshot so a reused instance re-applies the persisted local config. Commit `576bf86`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-20) — the transport is now declared the sole delivery extension seam. The `TaskProcessor` class docstring points subclassers at `TaskTransport` (injected via the keyword-only `transport=` argument) and lists the six hooks under a "Compatibility shims" section; each of the six hook docstrings (`fetch_tasks`, `return_task_to_queue`, `on_task_started`, `on_task_completed`, `_write_task_progress`, `_on_queue_drain_task_processing`) carries a "prefer overriding `TaskTransport.<method>`" note, with `fetch_tasks` documenting the precedence (an override bypasses the injected transport for intake). A new `tests/task_processor/test_extension_seam.py` pins the transport as the live seam, the documented override precedence, and the class-docstring guidance. Docs corrected: `docs/valkey_worker.md` no longer falsely claims `ValkeyWorker` overrides the hooks (it injects `ValkeyTransport`); `docs/task_processor.md`, `docs/task_handler.md`, `README.md`, and `docs/mqtt_worker.md` annotate the hooks as back-compat shims. Commit `14ce4f3`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-18) — removed the dead `release` method from the `TaskTransport` Protocol and all three transports (`InMemoryTransport`, `ValkeyTransport`, `MqttTransport`); added `refresh_leases()` and `recover_pending_tasks(sink) -> tuple[bool, bool]` as required Protocol members (`InMemoryTransport` implements them as a no-op and `(True, False)`). Documented the `on_drain` contract (release the claim unconditionally; durable transports leave the entry pending, non-durable re-deliver when `canceled_action == "requeue"`). Workers now call `self._transport.refresh_leases()` / `self._transport.recover_pending_tasks(self)` protocol-typed, no concrete down-cast. The Protocol now has 8 methods: `fetch`, `requeue`, `on_started`, `ack`, `on_progress`, `on_drain`, `refresh_leases`, `recover_pending_tasks`. Commit `2b9f66d`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-19) — extracted the shared field-population matrix into a new core module `src/scietex/service/task_status.py` with two pure builders (`build_running_status`, `build_terminal_status`); `TaskStatusStore.record_running`/`record_terminal` and `MqttTransport._publish_status`/`ack` now both delegate to them, and a parametrized cross-transport equivalence test pins the invariant; commit `a8db711`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-19) — `inbox_ttl` now defaults to `DEFAULT_INBOX_TTL = 86400` (one day) instead of `None` (unbounded); pruning is decoupled from `_load_entries` into a `MqttInbox.prune_expired()` maintenance pass invoked by a `MqttWorker.watchdog` override throttled to a fixed 60 s maintenance interval; the `_load_entries` prune call is removed. Commit `d071c6e`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-19) — a new permanent `CONFIG_SOURCE_NOT_CONFIGURED` code now represents "no source attached", so the transient "transport down / no config yet" condition is carried solely by `CONFIG_SOURCE_UNAVAILABLE`; the retryable set is centralized as `RETRYABLE_ERROR_CODES = frozenset({CONFIG_SOURCE_UNAVAILABLE})`, and `ConfigReloader.store` now maps a remote store failure to `CONFIG_SOURCE_UNAVAILABLE` (retryable during a network blip) while `CONFIG_STORE_FAILED` is local-disk-only. Commit `61072d1`.
- **Severity:** MEDIUM
...
- **Status:** ✅ RESOLVED (2026-09-20) — a separate declarative persistence/inspection view was added: `DeclarativeSettings`/`DeclarativeSections` + `to_declarative` in `config_reload.py`, `ConfigReloader.show_declarative()`/`apply_declarative_sections()`, and `ConfigManager.write_local()`/`apply_local_file()` now use the declarative view so a store→restart cycle preserves `None`/auto-tune intent; `config:show` gained a `declarative_settings` field while `settings` stays effective (AR-100 preserved). The remote desired-state envelope stays concrete by design (no `CONFIG_ENVELOPE_VERSION` bump). Commit `269be4c`.
- **Severity:** LOW
...
- **Status:** ✅ RESOLVED (2026-09-19) — corrected the `register_config_settings` docstring to state that section hooks run **before** the core swap (validate-before-swap), matching the reloader's actual ordering; the code was already correct, so this is a documentation-accuracy fix only; commit `430cff7`.
- **Severity:** LOW
...
- **Status:** ✅ RESOLVED (2026-09-19) — statuses are keyed by handler identity (the `AsyncLoggingHandler` instance) instead of `handler.name`/`__class__.__name__`, and non-async handlers are skipped in both `start_handlers` and `shut_down_handlers` (never inserted into `statuses`); three new tests pin the identity keying and the non-async skip. Commit `69def51`.
- **Severity:** LOW
...
- **Status:** ✅ RESOLVED (2026-09-19) — a shared `RecoverableTransport` mixin (`src/scietex/service/transport.py`) now owns the `recovered` class attribute and the `ensure_recovered(sink) -> bool` recovery-once guard; `ValkeyTransport` and `MqttTransport` inherit it and their duplicated `recovered` flags and inline guards are deleted; `MqttWorker.initialize()` no longer recovers eagerly, so recovery runs on the first `fetch` for both transports. Commit `e1d5de3`.
- **Severity:** LOW
...
- **Status:** ✅ RESOLVED (2026-09-19) — `TaskLeaseManager.acquire` now returns `False` (defer) on a glide connection error instead of `True`, so `ValkeyTransport.recover_pending_tasks` leaves the entry pending and reports recovery incomplete rather than enqueuing an entry this worker never claimed; the AR-120 `RecoverableTransport.ensure_recovered` guard then retries on the next `fetch`. Commit `0abd60f`.
...
- **Status:** ✅ RESOLVED (2026-09-19) — `TaskExecutor.cancel`'s queued branch now pops `_retry_attempts[target_id]` (symmetrically with the existing AR-104 `_timeout_requeues` pop) before acking, so a task requeued for retry and then cancelled while still queued no longer leaks its error-path retry budget; a regression test pins that only the target's budget is cleared. Commit `34b5187`.
- **Severity:** LOW
...
3. ~~**Extract a `TaskExecutor`/`TaskScheduler` from `TaskProcessor` (AR-101).**~~ ✅ **DONE (2026-09-18)** — `TaskExecutor` now owns the execution loop, watchdog, cancellation, and shutdown drain; `TaskProcessor` keeps the public API and thin delegators. This unblocks AR-102 and AR-104/AR-122.

### Priority 2 — Important
4. ~~**Extract a shared `TransportWorker` base (AR-102).**~~ ✅ **DONE (2026-09-18)** — `TransportWorker` extracted into core; `ValkeyWorker`/`MqttWorker` now extend it.
5. ~~**Reconcile the `TaskTransport` Protocol (AR-113).**~~ ✅ **DONE (2026-09-18)** — `release` removed; `refresh_leases`/`recover_pending_tasks` promoted; `on_drain` contract documented.
6. ~~**Extract a shared `TaskStatus` record builder (AR-114).**~~ ✅ **DONE (2026-09-19)** — `task_status.py` builders shared by `TaskStatusStore` and `MqttTransport`.
7. ~~**Extract a `ConfigManager` collaborator (AR-105)** and register `config:*` handlers only when enabled.~~ ✅ **DONE (2026-09-19)** — `ConfigManager` extracted into core; `config:*` handlers registered only when `remote_config_enabled=True`.
8. ~~**Bound the timeout-requeue loop (AR-104)** and unify the retry budget across error and timeout paths.~~ ✅ **DONE (2026-09-19)** — new restart-required `max_timeout_requeues` ceiling (default 1) with a distinct executor budget and terminal ack on the ceiling.
9. ~~**Reset reloader state per run (AR-111)** so the disk-persistence path survives instance reuse.~~ ✅ **DONE (2026-09-19)** — `ConfigReloader.reset()` + `apply_envelope(trusted=True)` + the `TaskProcessor.initialize()` run-boundary call restore the disk-persistence path on instance reuse.

### Priority 3 — Opportunistic
10. ~~**Add lifecycle transition guards and replace the 100 ms poll (AR-106).**~~ ✅ **DONE (2026-09-19)**
11. ~~**Converge manager registration on one path (AR-107).**~~ ✅ **DONE (2026-09-19)**
12. ~~**Set a finite `inbox_ttl` default and decouple pruning (AR-115).**~~ ✅ **DONE (2026-09-19)**
13. ~~**Unify the config error taxonomy (AR-116).**~~ ✅ **DONE (2026-09-19)**
14. ~~**Fix the `register_config_settings` docstring ordering (AR-118).**~~ ✅ **DONE (2026-09-19)**
15. ~~**Key logging statuses by identity (AR-119);**~~ ✅ **DONE (2026-09-19)** ~~**unify recovery orchestration (AR-120);**~~ ✅ **DONE (2026-09-19)** ~~**return `False` on lease-acquire error (AR-121);**~~ ✅ **DONE (2026-09-19)** ~~**pop the retry budget on queued cancel (AR-122);**~~ ✅ **DONE (2026-09-19)** ~~**use a dataclass for `TaskTracker` (AR-109);**~~ ✅ **DONE (2026-09-20)** ~~**document the `ConfigSource` freshness divergence (AR-110);**~~ ✅ **DONE (2026-09-20)** ~~**demote the legacy template hooks (AR-112).**~~ ✅ **DONE (2026-09-20)**
...
- `TransportWorker` (new core base) — owns the client-lock/reconnect pattern, `TransportHealth` construction, and the config-reload pipeline; concrete workers override `connect`/`disconnect`/`heartbeat`/registry. **Shipped in v4.5.0 (AR-102).**
- `TaskTransport` Protocol — the complete seam: the eight methods (`fetch`/`requeue`/`on_started`/`ack`/`on_progress`/`on_drain`/`refresh_leases`/`recover_pending_tasks`), `release` removed, with `refresh_leases`/`recover_pending_tasks` carrying explicit no-op/`(True, False)` defaults. **Shipped in v4.5.0 (AR-113).**
...
3. ~~**AR-101** (`TaskExecutor` extraction) — mechanical move of the `handle_task` closure + `task_manager` body behind an injected collaborator; keep `TaskProcessor`'s public API stable.~~ ✅ **DONE (2026-09-18)** — shipped in v4.5.0.
4. ~~**AR-102** (`TransportWorker` base) — extract after AR-101 so the base composes the new executor; migrate `ValkeyWorker` then `MqttWorker` one at a time.~~ ✅ **DONE (2026-09-18)** — shipped in v4.5.0.
5. ~~**AR-113** (Protocol reconciliation) — do together with AR-102, since the base will type against the completed Protocol.~~ ✅ **DONE (2026-09-18)** — shipped in v4.5.0.
6. ~~**AR-114** (status builder) — independent; can land any time after AR-102.~~ ✅ **DONE (2026-09-19)** — shipped in v4.5.0.
7. ~~**AR-105** (`ConfigManager`) — after AR-100; register handlers conditionally.~~ ✅ **DONE (2026-09-19)** — shipped in v4.5.0.
8. ~~**AR-104**~~ ✅ **DONE (2026-09-19)** — shipped in v4.5.0. ~~**AR-111**~~ ✅ **DONE (2026-09-19)** — shipped in v4.5.0. **AR-115 / AR-116** — independent behavior fixes; each small and separately testable.
...
| AR-101 | `TaskProcessor` god-component; scheduler inlined | HIGH | Untestable core; every feature lands in one class | Medium | 1 | ✅ resolved |
| AR-102 | `ValkeyWorker`/`MqttWorker` duplicate lifecycle scaffolding | HIGH | Fixes must be made twice; drift risk | Medium | 2 | ✅ resolved |
| AR-113 | `TaskTransport` Protocol incomplete (dead + missing members) | MEDIUM | Third transport must guess; watchdog not protocol-typed | Low | 2 | ✅ resolved |
| AR-114 | Task-status matrix duplicated across packages | MEDIUM | Silent observability drift between transports | Low | 2 | ✅ resolved |
| AR-105 | Remote config leaked into core `TaskProcessor` | MEDIUM | Reduced cohesion; dead handlers when disabled | Medium | 2 | ✅ resolved |
| AR-104 | Timeout-requeue uncapped, no shared budget | MEDIUM | Unbounded redelivery loop for always-timing-out tasks | Low | 2 | ✅ resolved |
| AR-111 | Reloader state instance-lifetime; restart drops local config | MEDIUM | Disk-persistence path broken on instance reuse | Low | 2 | ✅ resolved |
| AR-106 | Lifecycle: no transition guards, 100 ms poll, fire-and-forget | MEDIUM | Testability; busy-wait; cancellation race | Medium | 3 | ✅ resolved |
| AR-107 | Two manager-registration mechanisms; `Manager` conflates roles | MEDIUM | Cognitive cost; fragile discovery | Medium | 3 | ✅ resolved |
| AR-108 | Control-plane shares data-plane queue/concurrency | MEDIUM | Control plane unreachable under saturation | Medium | 3 | ✅ resolved |
| AR-109 | `TaskTracker` is a serializable struct holding a live Task | MEDIUM | Misleading type semantics | Low | 3 | ✅ resolved |
| AR-110 | `ConfigSource.load` freshness semantics diverge | MEDIUM | Leaky abstraction; per-transport guarantees differ | Low | 3 | ✅ resolved |
| AR-112 | Legacy template hooks compete with the transport seam | MEDIUM | Ambiguous extension contract; misuse risk | Low | 3 | ✅ resolved |
| AR-115 | MQTT inbox tombstone growth unbounded by default | MEDIUM | Disk growth; slower scans over time | Low | 3 | ✅ resolved |
| AR-116 | Config error taxonomy asymmetric (load retryable, store not) | MEDIUM | Inconsistent retry semantics for operators | Low | 3 | ✅ resolved |
| AR-117 | `store`/`show` emit resolved values, losing `None`/`auto_tune` | LOW | Lossy round-trip; frozen defaults | Low | 3 | ✅ resolved |
| AR-118 | `register_config_settings` docstring contradicts hook ordering | LOW | Service-side bugs from wrong contract | Low | 3 | ✅ resolved |
| AR-119 | Logging status keyed by name/class-name | LOW | Latent collision in bookkeeping | Low | 3 | ✅ resolved |
| AR-120 | `recover_pending_tasks` trigger points diverge | LOW | Recovery contract harder to reason about | Low | 3 | ✅ resolved |
| AR-121 | `TaskLeaseManager.acquire` returns `True` on connection error | LOW | Possible double-processing under interleaving | Low | 3 | ✅ resolved |
| AR-122 | `_retry_attempts` leaks on queued cancellation | LOW | Slow bounded leak; accounting smell | Low | 3 | ✅ resolved |
