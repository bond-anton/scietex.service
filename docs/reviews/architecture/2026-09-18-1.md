...
- **Valkey client ownership** is inconsistent: three collaborators use a late-bound provider, the config source pins a stale client (AR-103).
...
1. **Fix the stale `ValkeyConfigSource` client (AR-103).** Smallest change, closes the only HIGH-severity correctness gap. Convert to a late-bound `client_provider` (or re-attach in `_connect_locked`).
2. **Consolidate effective config into one representation (AR-100).** Remove the reloadable-field shadows; derive the allowlist from a single enumeration; eliminate the duplicated resolution logic. This unblocks AR-105 and AR-117.
...
1. **AR-103** (stale client) — isolated, <1 h, no API change. Ship first.
2. **AR-100** (config consolidation) — internal to `TaskProcessor`; prerequisite for AR-105/AR-117. Land with tests asserting `show`/`store`/apply agree.
...
|---|---|---|---|---|---|
| AR-103 | `ValkeyConfigSource` holds a stale client across reconnects | HIGH | Remote config silently dies after reconnect | Low | 1 |
| AR-100 | Effective config duplicated across struct/shadows/properties | HIGH | Drift on hot-reload path; change amplification | Medium | 1 |
