...
- **Status:** ✅ RESOLVED (2026-09-18) — extracted `TransportWorker` (`src/scietex/service/transport_worker.py`), a shared core base owning the client-lock/reconnect pattern (`connect`/`disconnect`/`_reconnect`), `TransportHealth` construction + `transport_health`, the startup config-apply pipeline (`_apply_local_config`/`_reload_remote_config`/`_read_remote_outcome`/`_log_config_outcome`), and the `watchdog` glue. `ValkeyWorker` and `MqttWorker` now extend it and keep only broker-specific `_connect_locked`/`_disconnect_locked`, `_read_remote_outcome`, `heartbeat`, `initialize`, `cleanup`, registry methods, and (MQTT) intake/message-loop methods. Exported from `__init__.py` (additive). New isolated `tests/transport_worker/` suite. Commit `pending` (to be filled in).
- **Severity:** HIGH
