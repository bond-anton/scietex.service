...
| Configuration | `src/scietex/service/config.py` | `WorkerConfig` + `TaskProcessorConfig` (immutable `msgspec.Struct`s) and the MIN/MAX/DEFAULT constants they enforce at construction (`msgspec.ValidationError` on out-of-range values); also hosts `prepare_conf_dir()` config-dir fallback search/creation |
| Task processing | `src/scietex/service/task_processor.py` | `TaskProcessor`: in-process bounded task queue, concurrency limit, handler registry/dispatch, timeout watchdog, built-in `task:cancel` cancellation and `worker:*` lifecycle control (auto-registered handlers + injected callbacks), drain/requeue on shutdown. Composes a `TaskTransport` (keyword-only `transport=`, default `InMemoryTransport`) |
...
```
        ...
        (supports/handle/start/stop)     (TaskData/TaskResult/...)
               ▲
               │ (delivery via composed TaskTransport)
...
- **Workers own the loop.** `BasicWorker` provides the only place where
...
- **Valkey transport is isolated** in the `valkey` subpackage; `ValkeyWorker`
  composes a `ValkeyTransport` (implementing the core `TaskTransport` Protocol)
  and injects it as `TaskProcessor._transport`. The six legacy delivery hooks
...
| `examples/progress_and_cancel.py` | `ValkeyWorker` + `TaskCapabilities.report_progress` + `task:cancel` | Reports granular progress and cancels a running task via the built-in handler |
...
| Per-task worker task (`handle_task` wrapper) | `TaskProcessor.task_manager` | task `handle()` returns/raises, or watchdog cancellation |
...
independent `GlideClient` (built from `valkey_config=`, AR-059/061);
