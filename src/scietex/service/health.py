...
the connection degraded and requests a reconnect. :meth:`TransportHealth.recover`
is the single place a reconnect is actually attempted: ``ValkeyWorker.watchdog``
and ``ValkeyTransport.fetch`` both funnel into it, so a burst of concurrent
failures still produces exactly one reconnect attempt, gated by a cooldown to
prevent storms (e.g. ``lease.refresh`` looping over N tasks).

This module deliberately imports no transport-specific types: a failure is
unclassified and every reported error requests a reconnect, with the cooldown —
not error classification — as the storm guard.
"""
