...
async def test_fetch_delivers_control_behind_full_data_lane():
    ...
    t1, t2 = uuid4(), uuid4()
    data = TaskData(task_id=str(t1), task="a")
    control = TaskData(task_id=str(t2), task=TASK_CANCEL_TASK_NAME)
    transport.submit(data)
...
async def test_fetch_preserves_data_fifo_when_control_interleaved():
    ...
    t1, t2, t3 = uuid4(), uuid4(), uuid4()
    d1 = TaskData(task_id=str(t1), task="a")
    c1 = TaskData(task_id=str(t2), task=TASK_CANCEL_TASK_NAME)
...
async def test_requeue_of_control_task_stays_on_control_deque():
    ...
    sink = FakeSink()
    t1 = uuid4()
    control = TaskData(task_id=str(t1), task=TASK_CANCEL_TASK_NAME)
    transport.submit_control(control)
    assert await transport.fetch(sink) is True
...
async def test_on_drain_of_control_task_with_requeue_stays_on_control_deque():
    ...
    transport = InMemoryTransport(logger=_logger())
    t1 = uuid4()
    control = TaskData(task_id=str(t1), task=TASK_CANCEL_TASK_NAME, canceled_action="requeue")
    transport.submit_control(control)
    assert await transport.fetch(FakeSink()) is True
    assert list(transport._control_pending) == []
