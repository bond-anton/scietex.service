...
async def test_recover_pending_tasks_enqueues_pending_entries():
    ...
    payload = encode_task_envelope(TaskData(task_id=str(t_id), task="dummy", payload=b"{}"))
    # xautoclaim returns [next_start, {entry_id: [[field, value]]}, [deleted_ids]]
    client = DummyClient(
        xautoclaim_result=[
            b"0-0",
            {b"9-0": [[TASK_FIELD, payload]]},
            [],
        ]
    )
...
async def test_recover_pending_tasks_incomplete_when_queue_full_leaves_recovered_false():
    ...
    payload_a = encode_task_envelope(
        TaskData(task_id="22222222-2222-2222-2222-222222222222", task="dummy", payload=b"{}")
    )
    payload_b = encode_task_envelope(
        TaskData(task_id="33333333-3333-3333-3333-333333333333", task="dummy", payload=b"{}")
    )
    ...
    client = DummyClient(
        xautoclaim_result=[
            b"0-0",
            {
                b"9-0": [[TASK_FIELD, payload_a]],
                b"9-1": [[TASK_FIELD, payload_b]],
            },
            [],
        ]
    )
...
async def test_recover_pending_tasks_reclaims_entry_when_lease_absent():
    ...
    assert not worker.task_queue_empty()
    dequeued_data = worker.dequeue_task()
    assert dequeued_data is not None
    assert task_data_id(dequeued_data) == t_id
...
async def test_recover_pending_tasks_lease_skip_and_reclaim_mixed():
    ...
    payload_leased = encode_task_envelope(TaskData(task_id=str(leased_id), task="dummy", payload=b"{}"))
    payload_free = encode_task_envelope(TaskData(task_id=str(free_id), task="dummy", payload=b"{}"))
    client = DummyClient(
        xautoclaim_result=[
            b"0-0",
            {
                b"9-0": [[TASK_FIELD, payload_leased]],
                b"9-1": [[TASK_FIELD, payload_free]],
            },
            [],
        ],
    ...
    dequeued_data = worker.dequeue_task()
    assert dequeued_data is not None
    assert task_data_id(dequeued_data) == free_id
    assert worker.task_queue_empty()
