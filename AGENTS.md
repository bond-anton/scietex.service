...
- **Control-plane scope:** control-plane commands are single-worker-scoped. `cancel_task` cancels only a task running on the worker that reads the command (a target on another worker returns `TASK_NOT_RUNNING`); `config:apply`/`config:store`/`config:show` affect only the reading worker, and other workers converge on their next startup/reload from the durable desired state. Cross-worker control routing is planned for v5.0.0 (durable unified control stream).
